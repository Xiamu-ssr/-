    ...
    for(int t = 0; t < nt; ++t) {
        ...
        for(int z = z_start; z < z_end; ++z) {
            for(int y = y_start; y < y_end; ++y) {
                for(int x = x_start+((nt+1)%2); x < x_end; x+=2) {
                    ...
                    a0[INDEX(x, y, z, ldx, ldy)] = a7  +  (b7 * c7) / (b7 + c7); //sqrt
                    b0[INDEX(x, y, z, ldx, ldy)] = b7  +  (a7 * c7) / (a7 + c7); //sqrt
                    c0[INDEX(x, y, z, ldx, ldy)] = c7  +  (a7 * b7) / (a7 + b7); //sqrt

                }
                for(int x = x_start+(nt%2); x < x_end; x+=2) {
