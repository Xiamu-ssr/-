...
int main(int argc, char **argv) {
    int nt, type, thread_scheme, status, status_check, p_id, p_id_check;
    ...
    MPI_Comm_split(MPI_COMM_WORLD, status_check, info.p_id, &check_procs);
    if(status_check == 0){
        MPI_Comm_rank(check_procs, &p_id_check);
    }
    ...
    start = MPI_Wtime();
    if (type == 7) {
        ans0 = stencil_7(a0, a1,b0, b1,c0, c1, &info, nt, active_procs);
    } else {
    ...
    if(status == 0) {
        if(argc == 8 && p_id == 0) {
            printf("MPI - %d %d: checking\n", info.p_id, p_id);
            check_result_t result;
            ans1 = (ans0 == a0) ? a1 : a0;
            // ABORT_IF_ERROR(write_file(ans0, argv[7], &helper, active_procs))
            ABORT_IF_ERROR(read_file(ans1, argv[7], &helper, check_procs))

            result = check_answer(ans0, ans1, &info, check_procs);
            if(p_id == 0) {
                printf("errors:\n    1-norm = %.16lf\n    2-norm = %.16lf\n ", \
