...
int main(int argc, char **argv) {
    ...
    printf(INDENT"%d x %d, %d non-zeros, Index of sparsity %.6lf%, %d run(s)\n", \
            mat.global_m, mat.global_m, mat.global_nnz, (double)(mat.global_nnz)*100/(double)(mat.global_m)/(double)(mat.global_m) ,reps);
    ...
    for(i = 0; i < reps; ++i) {
        // fprintf(stderr, "\e[1;31m"INDENT"ITER : %d START\e[0m\n", i);
        cudaMemset(y, 0, sizeof(data_t) * mat.global_m);

        gettimeofday(&start, NULL);
        spmv(&mat, x, y);
        cudaDeviceSynchronize();
        // fprintf(stderr, "\e[1;31m"INDENT"ITER : %d STOP\e[0m\n", i);
        gettimeofday(&stop, NULL);
...
void print_help(const char *argv0, int p_id) {
    if(p_id == 0) {
        printf("USAGE: %s <repetitions> <input-file>\n", argv0);
    }
}
...
int my_abort(int line, int code) {
    printf("33merror at line %d, error code = %d\n", line, code);
